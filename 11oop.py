...
# def SungJukProgram():   # 함수에 너무 많은 기능부여
#     pass                # 하나의 기능만 부여
...
class SungJuk:
    ...
    def __str__(self):
        result = f'{self.name} {self.kor} {self.eng} {self.mat} {self.tot} ' \
                 f'{self.mean} {self.grd}'
        return result

sj1 = SungJuk('혜교', 96, 98, 50)
print(sj1)
...
class Employee:
    ...
    def __str__(self):
        result = f'{self.empno} {self.lname} {self.fname} {self.email}' \
                 f'{self.phone} {self.hdate} {self.jobid} {self.sal} ' \
                 f'{self.comm} {self.mgr} {self.deptid}'
        return result
